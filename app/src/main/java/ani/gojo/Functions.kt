...
import ani.gojo.anilist.Genre
import ani.gojo.anilist.api.FuzzyDate
import ani.gojo.anime.Episode
import ani.gojo.databinding.ItemCountDownBinding
import ani.gojo.media.Media
import ani.gojo.others.DisabledReports
    ...
    CoroutineScope(Dispatchers.IO).launch {
        try {
            ...
            if (loadData<Boolean>("sd_dl") == true && arrayOfFiles.size > 1 && arrayOfFiles[0] != null && arrayOfFiles[1] != null) {
                ...
                val direct = File(Environment.DIRECTORY_DOWNLOADS + "/gojo/Anime/${aTitle}/")
                if (!direct.exists()) direct.mkdirs()
                request.setDestinationInExternalPublicDir(
                    Environment.DIRECTORY_DOWNLOADS,
                    "/gojo/Anime/${aTitle}/$name"
                )
            }
